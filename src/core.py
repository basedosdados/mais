...
class Base():
    ...
    def render_template(self, template_file, kargs):
        ...
        return Template((self.templates / template_file).open('r').read()).render(**kargs)
...
class Dataset(Base):
    ...
    def __init__(self, dataset_id):
        ...
        self.dataset_config = self.load_yaml(self.bases_path / dataset_id / 'dataset_config.yaml')
    ...
    def create_dataset_ids(self, create_staging):
        ...
        if create_staging:
            dataset_ids.append(f"{self.client.project}.staging_{self.dataset_config['dataset_id']}")
    ...
    def setup_dataset_object(self, dataset_id):
        ...
        dataset.description = self.render_template('dataset/dataset_description.txt', self.dataset_config)
    ...
    def update(self, update_raw=True):
        ...
        for ds_id in dataset_ids:
            ...
            # Raises google.api_core.exceptions.Conflict if the Dataset already
            # exists within the project.
            dataset = client.update_dataset(dataset, fields=['description'], timeout=30)  # Make an API request.
...
class Table(Base):
    ...
    def init(self, data_sample_path=None, replace=False):
        ...
        if isinstance(data_sample_path, str):
            if data_sample_path.split('.')[-1] == 'csv':
                ...
                raise NotImplementedError("Data sample just supports comma separated csv files")
        else:
            columns = ['<column-name>']
    ...
    def create(self, external=False, job_config_params=None):
        ...
        if isinstance(job_config_params, dict):
            ...
            job_config_params.update(dict(
                schema=self.load_schema(),
                destination_table_description=self.render_template('table/table_description.txt', 
                                                                   self.table_config)))
        ...
        load_job = client.load_table_from_uri(
            self.uri.format(dataset=self.table_config['dataset_id'], table=self.table_config['table_id']), 
    ...
    def update(self, mode=['staging', 'prod']):
        ...
        for m, table_name in self.table_full_name.items():
            ...
            if m in mode:
                ...
                table.description = self.render_template('table/table_description.txt', 
                                                                       self.table_config)
