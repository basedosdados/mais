...
setwd("~/basedosdados/tratamento/br_ibge_pnadc")
...
for (ano in 2012:2021) {
  ...
  for (trimestre in trimestres) {
    message("Iniciando o trimestre ", trimestre, " do ano ", ano)
    
    dir.create(paste0("output/microdados/ano=",ano,"/trimestre=",trimestre),
               showWarnings = TRUE)
    
    message("Baixando PNADC do ano ", ano, " e trimestre ", trimestre)
    df = get_pnadc(ano,
                   quarter=trimestre,
                   labels = FALSE,
                   design = FALSE)
    ...
    for (uf in unique(df$sigla_uf)) {
      message("Tratando para o estado ", uf, " do ano ", ano, " do trimestre ", trimestre)
