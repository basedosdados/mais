...
cnae = pd.read_csv(
    ...
    na_values="",
    index_col=None,
    dtype={"CNO": str, "CNAE": str},
)

vinc = pd.read_csv(
    "./input/cno_vinculos.csv",
    encoding="latin",
    skipinitialspace=True,
    na_values=("", "9999-01-31", "3021-05-01"),
    index_col=None,
    dtype={"CNO": str, "Qualificação do contribuinte": str, "NI do responsável": str},
)
...
cno["Data de inicio da responsabilidade"] = cno[
    "Data de inicio da responsabilidade"
].dt.date
cno["Data de registro"] = cno["Data de registro"].dt.date
cno["Data da situação"] = cno["Data da situação"].dt.date
# ----
cnae = cnae.astype(
    {"CNO": "string", "CNAE": "string", "Data de registro": "datetime64"}
)

cnae["Data de registro"] = pd.to_datetime(
    cnae["Data de registro"], format="%Y/%m/%d"
).dt.date
...
vinc["Data de início"] = pd.to_datetime(vinc["Data de início"], format="%Y/%m/%d")
vinc["Data de registro"] = pd.to_datetime(vinc["Data de registro"], format="%Y/%m/%d")
...
cnae.rename(
    columns={"CNO": "id_cno", "CNAE": "cnae_2", "Data de registro": "data_registro"},
    inplace=True,
)
...
id_rf = pd.read_csv("./tmp/id_mun_rf.csv").astype(
    {"id_municipio": "string", "id_tom": "string", "mun": "string"}
)

cno["id_municipio_rf"] = cno["id_municipio_rf"].str.lstrip("0")

cno = cno.join(
    id_rf[["id_municipio", "id_tom"]].set_index("id_tom"), on="id_municipio_rf"
)
