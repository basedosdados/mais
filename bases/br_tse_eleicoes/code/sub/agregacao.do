...
foreach ano of numlist 1994(2)2020 {
	...
	foreach subdir in `subdirs' {
		...
		collapse (sum) votos, by(turno tipo_eleicao id_municipio id_municipio_tse cargo sigla_partido numero id_candidato_bd resultado)
...
foreach ano of numlist 1994(2)2020 {
	...
	foreach subdir in `subdirs' {
		...
		collapse (sum) votos*, by(turno tipo_eleicao id_municipio id_municipio_tse cargo sigla)
...
foreach ano of numlist 1994(2)2020 {
	...
	*
	...
	collapse (sum) votos, by(turno tipo_eleicao sigla_uf id_municipio id_municipio_tse cargo sigla_partido numero id_candidato_bd resultado)
	
	order turno tipo_eleicao sigla_uf id_municipio id_municipio_tse cargo sigla_partido numero id_candidato_bd resultado votos
...
foreach ano of numlist 2020 { //1994(2)2020 {
	...
	foreach subdir in `subdirs' {
		...
		collapse (sum) aptos secoes secoes_agregadas aptos_tot secoes_tot comparecimento abstencoes votos_*, by(turno tipo_eleicao id_municipio id_municipio_tse cargo)
