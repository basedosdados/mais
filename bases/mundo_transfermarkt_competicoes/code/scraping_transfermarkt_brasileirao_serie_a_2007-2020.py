...
@author: Eduardo
"""
# pylint: disable=invalid-name,anomalous-backslash-in-string,redefined-outer-name,too-many-locals,too-many-branches,too-many-statements
import re
...
def main():
    ...
    df["idade_media_titular_vis"] = df["idade_media_titular_vis"].map(
        lambda x: str(x).replace(" ", "")
    )
    df["idade_media_titular_man"] = df["idade_media_titular_man"].map(
        lambda x: str(x).replace(" ", "")
    )

    df["valor_equipe_titular_man"] = df["valor_equipe_titular_man"].map(
    ...
    df["valor_equipe_titular_vis"] = df["valor_equipe_titular_vis"].map(
        lambda x: str(x).replace("m", "0000")
    )
    df["valor_equipe_titular_vis"] = df["valor_equipe_titular_vis"].map(
        lambda x: str(x).replace("Th.", "000")
