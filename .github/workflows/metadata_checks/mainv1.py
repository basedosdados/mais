...
def get_changes():
    ...
    metadata_files = [table_config for table_config in table_config_paths if Path(table_config).is_file()]
...
class Metadata:
    def __init__(self, change_path):
        ...
        self.yaml = yaml.load(open(Path(change_path), "r"), Loader=yaml.SafeLoader)

    def check_no_description(self):  
        ### check if any description is empty
        for column in self.yaml["columns"]:
            try:
    ...
    def check_table_dataset_id(self):  
        ###load table identifiers
        dataset_id = self.yaml["dataset_id"]
        table_id = self.yaml["table_id"]
        project_id_prod = self.yaml["project_id_prod"]
    ...
    def check_standard_columns(self):
    ...
    def check_update_frequency(self):
        ...
        try:
            assert self.yaml["data_update_frequency"] in allowed_update_freq
        except Exception as e:
            raise e
    ...
    def check_coverage(self):
        ...
        try:
            for field in self.yaml["coverage_geo"]:
...
def main():
    ...
    check_files = [Metadata(file) for file in metadata_files ]

    checks = [Metadata.check_abbreviation, Metadata.check_coverage, Metadata.check_no_description,
    Metadata.check_standard_columns, Metadata.check_table_dataset_id, Metadata.check_update_frequency]
