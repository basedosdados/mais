...
def write_contributions(user, users, contribution_types):
    text = ""
...
def write_reference(levels):
    ...
    for level, content in levels.items():
        conditions = re.sub("{|}|'", "", str(levels[level]["conditions"]).replace(",", " •"))
...
def write_markdown(
    ...
    username, 
    token, 
    contribution_types=["[infra]", "[dados]", "[docs]"]):
    
    text = emoji.emojize(f"""<h1>Contribuições</h1>
    <h2>Como funciona</h2>
    ...
    <table>
    {write_table(users,contribution_types)}
    </table>
    <br>
    """.replace("\n",""))
    ...
    repo.update_file(path="CONTRIBUTORS.md", 
